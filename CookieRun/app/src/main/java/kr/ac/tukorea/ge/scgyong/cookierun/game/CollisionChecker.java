...
public class CollisionChecker implements IGameObject {
    ...
    private final IBoxCollidable player;

    public CollisionChecker(MainScene mainScene, IBoxCollidable player) {
    ...
    public void update() {
            ...
            if (CollisionHelper.collides(player, item)) {
                ...
                if (item.index == 26) {
                    if (player instanceof Player) {
                        ((Player)player).magnify(true);
                    } else if (player instanceof NewPlayer) {
                        ((NewPlayer)player).magnify(true);
                    }
                }
                scene.remove(item);
            }
        ...
        for (int i = obstacles.size() - 1; i >= 0; i--) {
            ...
            if (CollisionHelper.collides(player, obstacle)) {
                if (player instanceof Player) {
                    ((Player)player).hurt(obstacle);
                } else if (player instanceof NewPlayer) {
                    ((NewPlayer)player).hurt(obstacle);
                }
            }
        }
    }
